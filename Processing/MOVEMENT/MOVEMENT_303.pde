...
void draw303() {
  ...
  m6 = max(m6 - 1, 0);
  sn = max(sn - 2, 0);
  xincr303 = max(xincr303 - 0.007, 0.02);
  print = max(print - 1, 0);
...
void perlinWaveLeft(color c, float x1, float x2) {
  ...
  // float xoff = yoff303; // Option #2: 1D Noise
  
  // Iterate over vertical pixels
  for (float y = 0; y <= width; y += 10) {
    // Calculate a x value according to noise, map to 
    float x = map(noise(xoff, yoff303), 0, 1, x1, x2); // width/4 - width/8, width/4 + width/8); // Option #1: 2D Noise
...
void perlinWaveRight(color c, float x1, float x2) {
  ...
  float xoff = 0;
  for (float y = 0; y <= width; y += 10) {
    float x = map(noise(xoff, yoff303), 0, 1, x1, x2); // width - (width/4 - width/8), width - (width/4 + width/8)); // Option #1: 2D Noise
    vertex(x, y); 
    xoff += xincr303;
  }
  yoff303 += 0.01;
