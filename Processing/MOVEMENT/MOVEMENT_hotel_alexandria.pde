...
Movie movieFuk;
float movieSpeedHotelAlexandria = 0.872;

float houseHotelAlexandria = 0;
...
void setupHotelAlexandria() {
  movieFuk = new Movie(this, "fuk.mov");
  //movieFuk.speed(movieSpeedHotelAlexandria);
  //movieFuk.play();
  
}

void drawHotelAlexandria() {
  blendMode(BLEND);
  if (movieFuk.available()) {
    movieFuk.read();
    if (movieFuk.time() > 24.5 && movieSpeedHotelAlexandria > 0) { // movie is 25.045s long
      movieSpeedHotelAlexandria = -1.0;
    } else if (movieFuk.time() < 23.5 && movieSpeedHotelAlexandria < 0 && aliceReverse == 0 && aliceAgain == 0) {
      movieSpeedHotelAlexandria = 1.0;
    } else if (movieSpeedHotelAlexandria < 0 && aliceReverse == 1) {
      movieSpeedHotelAlexandria = -0.25;
    } else if (movieSpeedHotelAlexandria < 0 && aliceAgain == 1) {
      ...
      movieSpeedHotelAlexandria = secondsTillDatamosh / 19.2; // about 19.2 seconds of build (8 cycles going from 80->120 cps)
      aliceAgain = 0;
    }
    movieFuk.speed(movieSpeedHotelAlexandria);
  }
  ...
  houseHotelAlexandria = max(0, houseHotelAlexandria - 6);
}
...
void mousePressedHotelAlexandria() {
  ////movieFuk.jump(10);
  //if (speed > 0) {
  //  speed = -1.0;
  //} else {
...
void oscEventHotelAlexandria(OscMessage m) {
  float t = millis();
  int i;
  ...
  switch(s) {
    case "house":
      if (gain > 0) {
        houseHotelAlexandria = int(60 * gain);
      } else {
        houseHotelAlexandria = int(60);
      }
      break;
    case "supersaw":
      if (orbit == 3) {
