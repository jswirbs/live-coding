...
boolean webcamInitialized = false;

// void setupWebcam() {  // can't run as setup as it'll mess with other phases
...
void uninitializeWebcam() {
  height = int(height*scale);
  width = int(width*scale);
}

void drawWebcam() {
  if (webcamInitialized == false) {
    initializeWebcam();
