...
void drawCircleWaves() {
  ...
  for (float i = 0; i < numRings; i++) {
    for (float j = 0; j < ringDensity; j++) {
      ...
      } else if (modeCircleWaves == 4 || modeCircleWaves == 5) {
        blendMode(BLEND); 
      ...
      if (modeCircleWaves == 1 || modeCircleWaves == 3) {
  ...
  if (modeCircleWaves == 5) {
    // scale the inverse of the scale above to keep it still
    scale(1/(2.2 + 1.5*sin(millis()*0.0005)));
    // harom triangle
    translate(-(width/2), -(height/2));
    fakeHarom();
    // harom(width/2+height/2.75 + sn, height-height/3.5 + sn/sqrt(3), width/2-height/2.75 - sn, height-height/3.5 + sn/sqrt(3), haromDepth, 0);
  } else {
    // scale the inverse (almost) of the scale above to keep it still (move less)
...
void fakeHarom() {
  float ax,ay,bx,by,vx,vy,nx,ny,cx,cy;
  ax = width/2+height/2.75 + sn;
  ay = height-height/3.5 + sn/sqrt(3);
  bx = width/2-height/2.75 - sn;
  by = height-height/3.5 + sn/sqrt(3);
  vx=bx-ax;
  vy=by-ay;
...
void mouseClickedCircleWaves() {
  if (mouseButton == LEFT) {
    ringDensity = 3;
    if (modeCircleWaves == 0) {
      modeCircleWaves = 1;
    } else {
      modeCircleWaves = 0;
    }
...
//void oscEventCircleWaves(OscMessage m) {
//  float t = millis();
//  int i;
